...
class Main {
  constructor(){
    ...
    let langauge=document.getElementById("x").value.trim().toLowerCase();
  ...
  createPredictBtn(){
    ...
    predButton.addEventListener('mousedown', () => {
      ...
      if(Math.max(...exampleCount) > 0){
        ...
        if(exampleCount[0] == 0){
          alert(
            `You haven't added examples for the wake word GOOGLE`
            )
          return
        }
